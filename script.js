...
var creators = [
  {
    ...
    bio: "Membuat Layout & tampilan website.",
    instagram: "@toga",
 ...
 
  {
    ...
    bio: "Mengumpulkan semua video pembelajaran.",
    instagram: "@desmonth",
 ...
 
  {
    ...
    bio: "Membuat materi Pembelajaran.",
    instagram: "@abin",
...
async function buildLatihanSection(fallbackSection) {
  try {
    ...
    // Gabungkan per judul — 1 topik bisa punya sampai 3 baris (easy/medium/hard)
    const grouped = {};
    data.forEach(function(row) {
      if (!grouped[row.judul]) {
        grouped[row.judul] = { judul: row.judul, urutan: row.urutan, deskripsi: row.deskripsi, levels: [] };
      }
      grouped[row.judul].levels.push(row.level);
      if (row.urutan < grouped[row.judul].urutan) grouped[row.judul].urutan = row.urutan;
    });
    const list = Object.values(grouped).sort(function(a, b) { return a.urutan - b.urutan; });
 
    return {
      icon: (fallbackSection && fallbackSection.icon) || '🎯',
      title: (fallbackSection && fallbackSection.title) || 'Latihan Soal',
      items: list.map(function(g) {
        return {
          num: 'Latihan',
          title: g.judul,
          description: g.deskripsi || (g.levels.length + ' level tersedia'),
          tag: 'Practice',
          file: 'latihan-viewer.html?modul=' + encodeURIComponent(modulFull) + '&judul=' + encodeURIComponent(g.judul)
        };
      })
    };
  } catch (e) {
