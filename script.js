...
function handlePageParam() {
    ...
    if (page) {
        ...
        // replaceHistory=true: tidak tambah entry baru, cukup replace state saat ini
        navigateTo(page, null, true);

    } else {
        // Default: beranda — render materi & game di background lalu tampilkan beranda
        renderMateri();
        renderGame();
        navigateTo('beranda', null, true);
    }
}
...
window.addEventListener('popstate', function(event) {
    if (event.state && event.state.page) {
        ...
        // replaceHistory=true: popstate sudah restore state, tidak perlu pushState baru
        navigateTo(page, null, true);
    } else {
        handlePageParam();
    }
...
var materiData = {
  ...
  aljabar: [{
    ...
    items:[
      ...
      {num:"02",title:"Matriks",description:"Operasi matriks, determinan, dan invers",tag:"Learn",file:"subject/aljali/Matriks.html"},
      ...
      {num:"04",title:"Nilai Eigen Dan Vektor Eigen",description:"Pemetaan linear dan representasi matriks",tag:"Learn",file:"subject/aljali/bab5.html"},
    ...
    items:[
      {num:"01",title:"Sistem Persamaan Linear",description:"Penyelesaian SPL dengan eliminasi Gauss dan metode substitusi",tag:"Watch",file:"video/aljali/Video_SPL.html"},
      {num:"02",title:"Matriks",description:"Operasi matriks, determinan, invers, dan sifat-sifatnya",tag:"Watch",file:"video/aljali/video_Matriks.html"},
      {num:"03",title:"Determinan",description:"Perhitungan determinan dengan ekspansi kofaktor dan eliminasi",tag:"Watch",file:"video/aljali/video_determinan.html"},
      {num:"04",title:"Nilai Eigen dan Vektor Eigen",description:"Pemetaan linear, eigenvalue, eigenvector, dan representasi matriks",tag:"Watch",file:"video/aljali/Video_Eigenvector.html"},
      {num:"05",title:"Diagonalisasi",description:"Diagonalisasi matriks menggunakan eigenvalue dan eigenvector",tag:"Watch",file:"video/aljali/Video_diagonalisasi.html"}
    ]
  },{
...
var gameData = [
  {icon:"🎮",title:"Game Matematika Diskrit",items:[
    ...
    {num:"02",title:"Graph Kingdom",description:"Bangun dan taklukkan kerajaan graf! Temukan jalur terpendek, hubungkan node, dan kuasai algoritma traversal graf.",tag:"Teori Graf",file:"game/matdis/graph_kingdom.html"},
    {num:"03",title:"Logic Detective",description:"Jadilah detektif logika! Pecahkan kasus menggunakan proposisi dan inferensi logika — siapa pelakunya?",tag:"Logika Proposisi",file:"game/matdis/logic-detective.html"},
    {num:"04",title:"Recursion Dungeon",description:"Selesaikan dungeon dengan kekuatan rekursi — setiap ruang tersembunyi adalah sub-masalah yang harus dipecahkan!",tag:"Rekursi",file:"game/matdis/recursion-dungeon.html"},
    {num:"05",title:"Set Collector Arena",description:"Kumpulkan elemen dan bentuk himpunan! Uji pemahamanmu tentang union, irisan, komplemen, dan selisih himpunan.",tag:"Teori Himpunan",file:"game/matdis/set-collector-arena.html"}
  ]},
  {icon:"👾",title:"Game Aljabar Linear",items:[
    {num:"01",title:"⚔ Matrix Battle Arena",description:"Adu kekuatan matriks dalam arena pertempuran! Kuasai penjumlahan, pengurangan, skalar, hingga perkalian matriks — 12 soal progresif.",tag:"Operasi Matriks",isNew:true,accentColor:"violet",file:"game/aljali/matrix-battle.html"},
    {num:"02",title:"Vector Racer",description:"Balapan vektor di lintasan multidimensi! Gunakan operasi vektor, dot product, dan normalisasi untuk memenangkan setiap ronde.",tag:"Vektor",isNew:true,accentColor:"violet",file:"game/aljali/matrix-battle.html"},
    {num:"03",title:"Transform City",description:"Transformasi kota dengan matriks! Rotasikan, refleksikan, dan skalakan bangunan menggunakan transformasi linear.",tag:"Transformasi Linear",isNew:true,accentColor:"violet",file:"game/aljali/transform-city.html"},
    {num:"04",title:"Determinant Crisis",description:"Selamatkan dunia dari krisis matriks singular! Hitung determinan dan selesaikan SPL sebelum waktu habis.",tag:"Determinan & SPL",isNew:true,accentColor:"violet",file:"game/aljali/determinan-crisis.html"},
    {num:"05",title:"Space Linear Mission",description:"Misi antariksa berbasis aljabar linear! Navigasi ruang vektor, hitung eigenvalue, dan diagonalisasi matriks untuk mendarat dengan selamat.",tag:"Eigenvalue",isNew:true,accentColor:"violet",file:"game/aljali/space-linear-mission.html"}
  ]},
  {icon:"🕹️",title:"Game Kriptografi",items:[
    {num:"01",title:"⚿ Hacker Cipher Wars",description:"Pecahkan perang sandi rahasia! Enkripsi dan dekripsi pesan menggunakan Caesar, Atbash, dan Vigenère — 10 level semakin menantang.",tag:"Enkripsi Klasik",isNew:true,accentColor:"green",file:"game/kriptografi/hacker-cipher-wars.html"},
    {num:"02",title:"⚿ Password Hunter",description:"Buru password yang tersembunyi! Gunakan teknik brute-force terkontrol dan analisis frekuensi untuk membobol sandi yang terkunci.",tag:"Analisis Sandi",isNew:true,accentColor:"green",file:"game/kriptografi/password-hunter.html"},
    {num:"03",title:"⚿ RSA Kingdom Defense",description:"Pertahankan kerajaan dengan tameng RSA! Pahami bilangan prima, fungsi Euler, dan enkripsi kunci publik untuk menghalau serangan.",tag:"Kriptografi RSA",isNew:true,accentColor:"green",file:"game/kriptografi/rsa-kingdom-defense.html"},
    {num:"04",title:"⚿ Secret Spy Mission",description:"Jalani misi mata-mata rahasia! Gunakan teknik steganografi dan sandi modern untuk menyembunyikan dan mengungkap pesan terselubung.",tag:"Steganografi",isNew:true,accentColor:"green",file:"game/kriptografi/spy-mission.html"},
...
/* ═══ NAVIGASI — Berpindah halaman ═══
   replaceHistory=true  → gunakan replaceState (tidak tambah entry baru)
   replaceHistory=false → gunakan pushState (tambah entry, default untuk klik user)
*/
function navigateTo(page, tab, replaceHistory) {
  // Jika tab diberikan, update state dulu
  if (tab) {
    if (page === 'materi') activeMateriTab = tab;
    if (page === 'game')   activeGameTab   = tab;
  }
  ...
  var activeTab = (page === 'materi') ? activeMateriTab : (page === 'game' ? activeGameTab : null);
  var newUrl = window.location.pathname + '?page=' + page + (activeTab ? '&tab=' + activeTab : '');
  var state = { page: page, tab: activeTab || null };

  if (replaceHistory) {
    // Saat load awal / popstate: ganti state saat ini, tidak tambah entry baru
    history.replaceState(state, '', newUrl);
  } else {
    // Saat klik user: tambah entry baru ke history
...
function openModuleFile(file) {
  // Simpan state dengan tab yang aktif agar tombol kembali di modul bisa kembali ke tab yang benar
  var stateUrl = window.location.pathname + '?page=materi&tab=' + activeMateriTab;
  history.replaceState({ page: 'materi', tab: activeMateriTab }, '', stateUrl);
...
function openGameFile(file) {
  ...
  var stateUrl = window.location.pathname + '?page=game';
  history.replaceState({ page: 'game', tab: null }, '', stateUrl);
...
function renderCreators() {
  ...
  creators.forEach(function(c) {
    ...
    html += '<div class="creator-role">' + c.role + '</div>';
    html += '<div class="creator-nim">NIM: ' + c.nim + '</div>';
    html += '<p class="creator-bio">' + c.bio + '</p>';
    html += '</div>';
    html += '</div>';
  });
...
(function init() {
  ...
  renderReferensi();
  renderCreators();
  // handlePageParam membaca URL, set tab yang benar, lalu memanggil navigateTo
  // dengan replaceHistory=true sehingga tidak ada history entry ganda
  handlePageParam();
})();
